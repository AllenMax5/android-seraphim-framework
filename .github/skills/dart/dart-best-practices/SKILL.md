...
- **Naming**: Follow [effective-dart](https://dart.dev/guides/language/effective-dart) (PascalCase
  classes, camelCase members).
...
- ❌ `var result = fetchOrders()` — use `final` or an explicit type; `var` only for locally obvious
  types in short scopes
- ❌ `import 'package:myapp/features/orders/order_model.dart'` within the same package — use relative
  imports for intra-package files
- ❌ Top-level mutable `List<Order> cachedOrders = []` — no global mutable state; use DI or
