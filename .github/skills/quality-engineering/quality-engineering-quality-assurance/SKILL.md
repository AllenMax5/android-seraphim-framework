...
- **Pattern**: `Platform_Module_Action on Screen when Condition` (e.g., `Web_Order_Verify...` or
  `Mobile_Order_Verify...`)
- **Rule**: Only include `Web_` or `Mobile_` prefix if requirement is exclusive to one platform.
  Omit prefix if it supports **Both**.
...
- ❌ Testing Web and Mobile behavior in a single TC when behavior diverges — split into separate TCs
  per platform
- ❌ `Order_Verify page` — name must follow the full pattern:
