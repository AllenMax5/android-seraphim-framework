...
- **Typing**: Avoid magic strings. Use a type-safe getter helper or a dedicated configuration
  object/interface.
...
1. **`src/config/env.validation.ts`**: Add the class property with appropriate `class-validator`
   decorators.
...
4. **CI/CD Pipelines & Infrastructure**: You **MUST** map the new variable in your deployment
   scripts (e.g., `.github/workflows/*.yml`, `gitlab-ci.yml`, Terraform, or Azure Pipelines). Most
   modern cloud platforms (Cloud Run, ECS, Kubernetes) require explicit mapping of secrets/env-vars
   into the container runtime. Failure to do this will cause the production deployment to crash or
   silently fail.
...
- **No new vars without CI/CD update**: Always update `env.validation.ts`, `.env.example`, and
  pipeline manifests.
