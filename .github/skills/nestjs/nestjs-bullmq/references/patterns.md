...
This file is the **single source of truth** for all BullMQ numeric options. It lives in
`src/common/constants/` and is imported by every processor and module.
...
BullMQ workers emit unhandled exceptions on Redis failure (like Upstash rate limits). Extend
`BaseProcessor` to throttle error logs and prevent log spam.
...
When Redis is unavailable the `ThrottlerGuard` (global `APP_GUARD`) must **not** kill all HTTP
routes. The `RedisThrottlerStorage` must catch all Redis errors and return a pass-through record.
...
**Never remove the processor because a cron exists.** The cron _schedules_; the processor
_executes_. Both are always required.
