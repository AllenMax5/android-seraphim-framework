...
Use `nestjs-pino` for high-performance, async JSON logging. Node's `console.log` is blocking and
unstructured.
...
- **Request ID**: Every log line **must** include a `reqId`. `nestjs-pino` handles this via
  `AsyncLocalStorage`.
- **Propagation**: Pass `x-request-id` to downstream microservices and database queries for
  end-to-end tracing.
...
- **No perf data in production by default**: Gate benchmarking behind `ENABLE_PERFORMANCE_BENCHMARK`
  flag.
