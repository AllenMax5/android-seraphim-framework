...
2. **Interceptor**: Maps low-level errors to HTTP Exceptions (e.g.,
...
- **Custom**: Extend `HttpException` only for domain-specific failures that need specific status
  codes.
...
- **Sanitization**: Ensure `ApiException` payloads do not leak internal file paths or raw variable
  dumps.
...
- **No HttpException in services**: Throw domain errors in services; let Interceptors map to HTTP
  exceptions.
