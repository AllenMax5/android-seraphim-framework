...
- **Upgrade Detection**: Always check `package.json` for versions of `next`, `react`, and
  `react-dom`.
- **Planning**: For major version jumps (v13 to v15), perform an incremental upgrade (v13 -> v14,
  then v14 -> v15). Follow the official Next.js Migration Guides.
- **Automated Codemods**: Use `npx @next/codemod@latest <transform> <path>` to automate syntax
  migration.
- **Breaking Changes (v15)**: Respond to the `next-async-request-api` transform by ensuring
  `params`, `searchParams`, `cookies()`, and `headers()` are awaited.
- **React Parity**: Upgrade `react` and `react-dom` to match Next.js peer dependencies (e.g., React
  19 for Next.js 15).
- **Validation**: Run `next dev` and `next build` after each incremental step. Check console errors
