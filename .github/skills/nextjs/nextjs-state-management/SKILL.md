...
Wrap `localStorage` reads in `useEffect` or a `mounted` flag to avoid hydration mismatches. Manage
optimistic updates with `useOptimistic` in Next.js 15+.
...
- Avoid adding Zustand or TanStack Query on top of an existing Redux codebase — migrate
