...
Centralize all data access (Database & External APIs) to ensure consistent security, authorization,
and caching.
...
- **Security**: Use `taintObjectReference` or `taintUniqueValue` from the experimental taint API to
...
- **Error Handling**: Throw standardized errors (`NotFoundError`, `UnauthorizedError`) caught by
  `error.tsx` or `notFound()`.
