...
- **Never Force Unwrap**: Use **guard let**, **if let**, or **nil coalescing (??)** to safely
  unwrap.
...
- **Composition over Inheritance**: **Prefer protocol composition** with structs for better
  decoupling and performance.
- **Extensions**: Implement **conformance** in an extension: *
  *`extension MyType: MyProtocol { ... }`**. Use extensions for clean code organization, not
  storage (do **not** use extensions for **stored properties**).
...
- **Enums**: Prefer enums with **associated values** over multiple Optionals. Extract values via a *
  *switch** statement (e.g., `case .success(let value):`).
- **Value Types**: **Default to struct** for **value semantics** and thread safety. Use `class` only
  when **reference identity** is necessary or for inheritance. Structs are copied; classes are
  shared.
