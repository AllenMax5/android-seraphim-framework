...
- **RLS**: `typeorm migration:generate` cannot detect Row-Level Security. Use raw
  `queryRunner.query()` SQL for RLS.
...
- **No synchronize in production**: Always run explicit migrations; `synchronize: true` is
  destructive.
