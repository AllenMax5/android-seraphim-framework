...
The target table **cannot** be referenced inside a `JOIN` within the `FROM` clause. Use a
comma-separated `FROM` list to avoid unexpected `INNER JOIN` behavior or syntax errors.
...
Always use `TIMESTAMPTZ`. Standard `TIMESTAMP` ignores the session timezone and can lead to silent
data corruption when moving between servers or handling DST.
...
- **Note**: `NOT (val = 'x')` will skip rows where `val` is `NULL`. Use `IS DISTINCT FROM` for
...
- **Fix**: Use a functional index `CREATE INDEX ... ON table (LOWER(column))` or use the `pg_trgm`
  extension for GIN/GIST index support on pattern matching.
