...
- **BRIN (Block Range Index)**: For very large tables with naturally ordered data (dates, IDs). Tiny
  index size.
- **Partial Indexes**:
...
- **Drop vs Delete**: `DROP TABLE partition_2023` is instantaneous;
  `DELETE FROM table WHERE year=2023` is slow and generates bloat.

## 5. Concurrency & Locking

- **Transaction Isolation**:
    - **Read Committed (Default)**: Sees data committed before the _query_ began.
    - **Repeatable Read**: Sees data committed before the _transaction_ began. Use for complex
