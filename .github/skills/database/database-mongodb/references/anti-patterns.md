...
- **Regex without Anchor**:
    ...
    - **Do**: `find({ name: /^john/ })` (Starts With) uses Index. Or use `$text` search / Atlas
      Search.
- **Redundant Indexes**:
    - **Don't**: Create index `{ a: 1 }` if `{ a: 1, b: 1 }` exists.
    - **Why?**: The compound index `{ a: 1, b: 1 }` can already support queries on `a`. Extra index
      wastes RAM and slows writes.
- **Negation Operators**:
