...
Network partitions are inevitable in distributed systems — **P is non-negotiable**. The real
trade-off is **C vs A**.
...
2. **Conflict resolution**: Define a strategy — Last Write Wins (LWW), vector clocks, CRDTs, or
   application-level merge.
3. **Read-your-writes**: If a user writes data, they must see their own write. Use sticky sessions
   or read from the primary for the writer's session.
