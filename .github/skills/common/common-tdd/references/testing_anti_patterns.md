...
- **Fix**: Test real component output or unmock the dependency. If isolation is required, assert on
  the _host's_ behavior, not the mock's existence.
...
- **Fix**: Mirror the real API/Object structure completely to prevent silent failures in realistic
  scenarios.

### 4. Over-Mocking

- **Violation**: Mock setup is >50% of the test file.
- **Fix**: Use true Integration Tests with real (but fast) dependencies. Complex mocks indicate
  over-coupling or poor test strategy.
