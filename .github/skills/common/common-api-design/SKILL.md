...
Consistent, predictable API contracts reduce integration friction and prevent breaking changes from
reaching consumers.
...
- `409` conflict; `422` business rule violation; `429` rate limit (add `Retry-After`); `500`
  unhandled.
...
- **Hierarchy**: Use nesting only up to 2 levels: `/users/:id/orders` ✅,
...
- Prefer cursor-based (`cursor` + `limit`) for large/live datasets; offset only for small static
  ones.
...
- **No breaking changes without versioning**: Removing/renaming fields in-place breaks consumers
  silently.
