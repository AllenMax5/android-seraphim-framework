...
Use the following manifest files to detect the project framework and its standard directory
structure.
...
> [!IMPORTANT]
> **Record `$SRC`, `$TEST`, and `$EXT` now.** Every subsequent scan uses these variables. Running
> against a wrong or non-existent directory will return empty results.
