...
**Fix**: Always append `AND owner_id = currentUser.id` (or tenantId) alongside the user-supplied
key.
...
**Fix**: Always project to a response DTO; use `@Exclude()` or explicit field selection; allowlist
writable fields.
...
**Fix**: Validate all third-party responses against a schema (e.g., Zod); treat external data as
untrusted input.
