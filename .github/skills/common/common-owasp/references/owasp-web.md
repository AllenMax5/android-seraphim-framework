...
**Fix**: Every resource query must include `AND owner_id = currentUser.id` or equivalent tenant
filter.
...
**Fix**: Use parameterized queries / prepared statements; never concatenate user input into queries
or shell arguments.
...
**Fix**: Validate and allowlist target URLs; block private IP ranges (RFC1918, loopback, metadata
endpoints).
