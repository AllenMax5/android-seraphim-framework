...
- Write 2–3 `evals` (should-trigger) per skill. Use prompts a real user would type — specific, with
  context, not abstract.
- Write 8–10 `should_not_trigger` entries. Focus on near-misses that share keywords but belong to a
  different skill.
...
**Good**:
`"ok so my boss gave me this ts file and it has like 5 different things going on in one class"`
...
**Good**: `"review this PR and check if the architecture looks right"` (code-review skill, not
skill-creator)
...
The description field is the **primary trigger mechanism**. Agents decide whether to use a skill
based solely on name + description.
...
3. **Cover edge cases**: Add contexts the agent might miss (e.g., "even if the user doesn't say X
   explicitly").
