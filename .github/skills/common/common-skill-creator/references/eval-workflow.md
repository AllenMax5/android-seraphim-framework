...
Test skills with parallel subagents — one with-skill, one without — to measure improvement
objectively.
...
When each subagent finishes, save to `timing.json` immediately — this data only comes through the
task notification once:
