...
Use before writing SKILL.md for unfamiliar or non-engineering domains (marketing, SEO, finance,
legal, store optimization, etc.).
...
**Rule**: Extract the 20% of findings that cover 80% of real use cases. Skip edge cases on first
pass.

## Non-Engineering Trigger Design

Engineering skills use file globs (`**/*ViewModel.kt`). Non-engineering skills must use
conversational triggers only.
...
**Target**: 5-8 specific intents in description. Cover formal phrasing, casual phrasing, and domain
jargon.
...
- **If skill requires web-search at runtime**: note this in SKILL.md as "Claude only" or split into
  a research reference and a separate action skill.
