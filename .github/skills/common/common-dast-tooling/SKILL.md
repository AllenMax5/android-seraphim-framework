...
- **No Scanning Production**: Never run DAST tools against live production environments. Use local
  or staging replicas only.
- **No Uncapped Scans**: Always set `max-depth` or `max-duration` to avoid infinite loops on dynamic
  routes.
- **No Anonymous Probing**: Use authenticated headers (`Authorization`) to test protected surfaces,
...
- **Bypassing Guards**: Probe protected routes with manipulated headers (`X-Forwarded-For`,
  `X-Custom-Auth`).
- **Data Leakage**: Request `/metrics`, `/health`, or `.git` directories to find exposed metadata.
- **Parameter Tampering**: Modify payload types (String -> Object) or inject large payloads to test
  limits.
...
- **No ignoring non-web protocols**: Check Docker ports, SSH banners, and internal gRPC/RMQ
  listeners.
