...
- **Don't try to model the entire system in a single diagram.** It leads to "ugly," unreadable
  messes.
- **Use Abstraction Layers:** Follow the C4 model (Context -> Container -> Component) to separate
  concerns. Each diagram should answer a specific set of questions for a specific audience.
...
- **Shapes:** Use the same shape for the same concept across all diagrams (e.g., Cylinder =
  Database, Person shape = User).
...
- **Whitespace:** Leave breathing room. Crowded diagrams imply a lack of clarity in the system
  design itself.
...
- **The "Orphan" Box:** Every node must be connected to something. If it's isolated, why is it
  there?
...
- **Inconsistent Abstraction:** Showing a "Database" box next to a "Class" box. Keep abstraction
  levels consistent.
