...
- **Composition**: Extract complex UI into small widgets. Avoid deep nesting or large helper
  methods.
- **Layout**:
...
- ❌ `Widget _buildHeader() { … }` helper methods — extract to a `const StatelessWidget` for proper
  rebuild control and DevTools profiling
- ❌ Accessing a controller or state directly in a widget (`_controller.data`) — use BLoC/Signals to
