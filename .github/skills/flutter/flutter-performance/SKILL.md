...
- **Repaints**: Use `RepaintBoundary` for complex animations. Use `debugRepaintRainbowEnabled` to
  debug.
...
- ❌ `setState()` at the root/page level to update a single counter — use `BlocBuilder` with
  `buildWhen` or `context.select()` for granular rebuilds
- ❌ Sorting/filtering a list inside `build()` — move heavy computation to BLoC or use `compute()`
- ❌ Non-`const` leaf widgets that never change — always apply `const` to static widgets to skip
  reconciliation
- ❌ `Column(children: items.map((e) => ItemWidget(e)).toList())` for large lists — use
