...
Strict guidelines for mock classes. Use shared mocks for all cross-feature components — no
duplication.
...
Do NOT define `MockMyBloc`, `MockMyRepository` in individual test files. Shared components (Blocs,
Repositories, Services) use shared mocks only.
...
Prohibit `any()` / `anyNamed()` — bypass type safety, cause silent failures. Use specific values or
typed matchers.
