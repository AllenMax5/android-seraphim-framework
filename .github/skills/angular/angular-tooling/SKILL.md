...
- **Flags**: Use `--dry-run` to preview output first. Add `--change-detection=OnPush` to set CD
  strategy at generation time. Use `--skip-tests` if spec is not needed.
...
- `--dry-run` — Preview output without writing files. Always use `--dry-run` first for unfamiliar
  generators.
...
- **Coverage**: `ng test --code-coverage --watch=false`. Coverage output goes to `coverage/`
  directory.
...
- **Note**: Check `angular.json` budgets — do not hand-edit `angular.json budgets` values; analyze
  the bundle first to understand what's large.

## Update Angular

- **Command**: `ng update` to check available updates.
- **Apply**: `ng update @angular/core @angular/cli` — this runs official **codemods** to migrate
  your code.
